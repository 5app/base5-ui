...
So, one might think, why not just trigger the browser's back button using `history.back`? The answer is that we need back buttons to have a sensible fallback for when there's no browser history yet, for example after a link has been opened in a new window, or when a page was reached via a link in an email.
...
For visual customisation, you can pass a `baseComponent` prop which changes the component that's rendered by default ([`ButtonCore`](src-button-core-readme)). Please make sure that this component has support for changing the rendered element type using the '`as`' prop (by default this is the case for any components built using styled-components).
