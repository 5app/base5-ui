...
1. We limit the number of theme variables that each design system component can _"know about"_ to a minimum.
...
For these use cases we came up with "Color Blocks": Simple mappings between a concept and a colour, e.g. `positive: 'green', negative: 'red', 'asset-video': 'yellow'`. Theese and can be used by UI components to add splashes of colour where necessary, and can also be customised for clients when needed.
